...
const DesktopWorkshopCarousel: React.FC = () => {
  const [current, setCurrent] = useState(0);
  const workshopsPerPage = 3; // Adjust the number of workshops per slide
  const length = workshops.length;
  const maxIndex = Math.ceil(length / workshopsPerPage) - 1;
...
export default DesktopWorkshopCarousel;
